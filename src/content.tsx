...
const insertReactRoot = (el: Element | null, position: 'before' | 'after', { className, tagName }: { className?: string, tagName?: string } = {}) => {
  if (!el) return
...
const appendReactRoot = (el: Element | null, { className, tagName }: { className?: string, tagName?: string } = {}) => insertReactRoot(el, 'after', { className, tagName })
const prependReactRoot = (el: Element | null, { className, tagName }: { className?: string, tagName?: string } = {}) => insertReactRoot(el, 'before', { className, tagName })
  ...
  return <div style={{ marginLeft: '-0.5em' }}>
    <div style={{ display: 'flex', alignItems: 'baseline' }}>
      <div className="flex-shrink-0 v-align-top color-fg-default Box-row--drag-button js-drag-handle"> 
          <svg style={{ width: 12 }} aria-hidden="true" height="16" viewBox="0 0 16 16" version="1.1" width="16" data-view-component="true" className="octicon octicon-grabber v-align-text-bottom">
      ...
      <div style={{ marginLeft: '0.3em'}}>
        <div style={{ display: 'flex', alignItems: 'baseline' }}>
          ...
          <span className="ml-2" style={{ color: 'var(--fgColor-muted, var(--color-fg-muted))' }}>0h</span>
          <a onClick={() => setShowOptions(!showOptions)} className="Box-row--drag-button pl-1 pr-1" style={{ 
        ...
        {showOptions && <div>
          ...
          <div><a onClick={() => {
            ...
            }} className='color-fg-danger btn-link mt-2'>Delete</a></div>
        </div>}
      </div>
    </div>
  </div>
...
appendReactRoot(document.getElementById('js-issues-toolbar')!.querySelector('.table-list-filters .table-list-header-toggle'), { tagName: 'span' })?.render(
  <React.StrictMode>
    <a onClick={() => {
    ...
    }} className='btn-link' style={{ marginLeft: '1em', padding: 0 }}>
      + Add group
    </a>
  </React.StrictMode>
)
...
const renderMilestoneGroups = () => {
  ...
  store.getState().map((_, i) =>
    prependReactRoot(container, { className: 'milestone-group Box-row Box-row--focus-gray js-navigation-item js-issue-row js-draggable-issue sortable-button-item' })?.render(
      <React.StrictMode>
        <Heading index={i} />
      </React.StrictMode>
    )
